...
> Раньше — кодовые имена `VOG-ZAVTRA` / `Vograph` / `ЗАПАРА`. Кодовый неймспейс `Vograph.*` и пакет `ru.bgtu_voenmeh.zapara` сохранены ради истории и автообновления.
...
- **Автообновление** — само проверяет релизы на GitHub при старте, скачивает и ставит (Android: открывает установщик, Windows: перезапуск через `update.bat`). Отключается в настройках.
